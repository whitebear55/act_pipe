...
from aloha.constants import (
    ...
    USB_CAM_IMAGE_TOPIC_NAME,
    CAM_SIDE2_IMAGE_TOPIC_NAME,
    DT,
    IS_MOBILE,
)
...
class ImageRecorder:
    def __init__(
        ...
        # ★ 수정: camera_names가 주어지면 그대로 사용, 없으면 기존 기본값 유지
        ...
        for cam_name in self.camera_names:
            ...
            elif cam_name == 'cam_side':              # ★ 추가
                callback_func = self.image_cb_cam_side
            elif cam_name == 'cam_side2':              # ★ 추가
                callback_func = self.image_cb_cam_side2
            else:
                raise NotImplementedError
            
            if cam_name == 'cam_side':
                topic = USB_CAM_IMAGE_TOPIC_NAME.format(cam_name)
            elif cam_name == 'cam_side2':
                topic = CAM_SIDE2_IMAGE_TOPIC_NAME.format(cam_name)
            else:
                topic = CAM_SIDE2_IMAGE_TOPIC_NAME.format(cam_name)
    ...
    def image_cb_cam_side2(self, data):
        cam_name = 'cam_side2'
        # 해상도 확인
        print(f"[{cam_name}] Incoming Resolution: {data.width} x {data.height}")
        return self.image_cb(cam_name, data)
    
    def image_cb_cam_side(self, data):                # ★ 추가
        return self.image_cb('cam_side', data)
