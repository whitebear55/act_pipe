...
bool PortHandlerMac::setCustomBaudrate(int speed)
{
  if(socket_fd_ < 0)
  {
    printf("[PortHandlerMac::SetCustomBaudrate] Serial port is not open!\n");
    return false;
  }

  if(speed <= 0)
  {
    printf("[PortHandlerMac::SetCustomBaudrate] Invalid baudrate: %d\n", speed);
    return false;
  }

  speed_t speed_value = (speed_t)speed;
  if(ioctl(socket_fd_, IOSSIOSPEED, &speed_value) == -1)
  {
    printf("[PortHandlerMac::SetCustomBaudrate] Failed to set baudrate %d (errno: %d)\n", speed, errno);
    return false;
  }
